...
echo -e "${GREEN}Starting setup for BTC Quantum script on Ubuntu/WSL...${NC}"
...
# Install Miniconda (for better environment management)
...
echo "Creating and activating Conda environment 'eth_env'..."
...
touch database/btc_addresses.db
...
# Script version
SCRIPT_VERSION = "1.1 - Fully Automated Bash Conversion"
...
DATABASE_PATH = os.getenv('DATABASE_PATH', 'database/11_13_2022/')
...
def worker(passphrases: List[str], local_bits: int, ibm_bits: int, checked_count: multiprocessing.Value):
    try:
        ...
        conn = sqlite3.connect('database/btc_addresses.db', check_same_thread=False)
...
if __name__ == "__main__":
    multiprocessing.set_start_method('spawn')  # Required for Windows, safe for Linux
    main()
EOF
...
echo -e "${GREEN}Running main.py in debug mode...${NC}"
python3 main.py

# Optional: Uncomment to run in full mode after debug
# echo "Editing main.py to disable debug mode..."
# sed -i 's/debug = True/debug = False/' main.py
# echo -e "${GREEN}Running main.py in full mode...${NC}"
# python3 main.py

echo -e "${GREEN}Setup and execution complete!${NC}"
echo "To reactivate the environment later, use: conda activate eth_env"
